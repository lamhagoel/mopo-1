...
class CQLPolicy(nn.Module):
    ...
    def learn(self, data):
        ...
        random_actions = torch.rand(num_samples_for_estimation, actions.shape[0], actions.shape[-1]).to(actions) * 2 - 1
        ...
        random_next_actions = torch.rand(num_samples_for_estimation, actions.shape[0], action.shape[-1]).to(action) * 2 - 1
        ...
        beta_loss = - torch.mean(torch.exp(self.log_beta) * (q1_penalty - lagrange_threshold).detach()) - 
                        torch.mean(torch.exp(self.log_beta) * (q2_penalty - lagrange_threshold).detach())
